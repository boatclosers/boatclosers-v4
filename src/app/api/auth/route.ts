...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (dealId) {
      ...
      // ── Step data (due diligence + documents) ── union so one side's save never
      // wipes the other side's progress. The maps are keyed by document/field id
      // and are effectively role-disjoint (each party signs/fills its own), so a
      // per-key union is safe. For top-level due-diligence flags we never let an
      // incoming empty value overwrite an existing real one (protects the buyer's
      // checklist from the seller's defaults, and vice-versa).
      const exDocs = existingRow.docs_data || {}
      const inDocs = docs_data || {}
      mergedPayload.docs_data = {
        ...exDocs, ...inDocs,
        signedDocs: { ...(exDocs.signedDocs || {}), ...(inDocs.signedDocs || {}) },
        docChecks: { ...(exDocs.docChecks || {}), ...(inDocs.docChecks || {}) },
        docFields: { ...(exDocs.docFields || {}), ...(inDocs.docFields || {}) },
      }
      const exDd = existingRow.dd_data || {}
      const inDd = dd_data || {}
      const mergedDd: any = { ...exDd, ...inDd }
      const isEmpty = (x: any) => x === undefined || x === null || x === false || x === '' || (Array.isArray(x) && x.length === 0)
      for (const k of ['survey','title','insurance','seatrial','engines','electronics','vaSigned','vaSigName','outcome','rejectionReasons','rejectionNotes','rejSolution','rejSigName','surveyCompany','surveyorName','surveyFile','proposedNewPrice','proposedNewPriceReason','ddNotes']) {
        if (isEmpty(inDd[k]) && !isEmpty(exDd[k])) mergedDd[k] = exDd[k]
      }
      mergedDd.sellerPrep = { ...(exDd.sellerPrep || {}), ...(inDd.sellerPrep || {}) }
      mergedPayload.dd_data = mergedDd
